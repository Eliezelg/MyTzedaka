...
let MultiTenantStripeService = MultiTenantStripeService_1 = class MultiTenantStripeService {
    ...
    async confirmPayment(tenantId, paymentIntentId, paymentMethodId) {
        const stripe = await this.getStripeInstance(tenantId);
        const params = {};
        if (paymentMethodId) {
    ...
    async updateConnectAccountStatus(stripeAccountId, account) {
        try {
            ...
            this.logger.error(`Erreur mise à jour statut compte Connect ${stripeAccountId}: ${error.message}`, error.stack);
            throw error;
        }
    }
};
